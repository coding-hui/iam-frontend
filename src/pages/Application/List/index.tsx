...
import CreateAppModal from '@/pages/Application/components/CreateAppModal';
import {
  deleteApplication,
  listApplications,
...
const INTL = {
  ...
  DISPLAY_NAME: {
    id: 'app.table.displayName',
  },
  LOGO: {
    id: 'app.table.logo',
  },
  HOMEPAGE_URL: {
    id: 'app.table.homepageUrl',
  },
  DESCRIPTION: {
...
const AppList: React.FC = () => {
  ...
  const { run: doDeleteApp } = useRequest(deleteApplication, {
    manual: true,
    onSuccess: () => {
      reloadTable();
  ...
  const handleListApps = async (opts: ListApplicationsOptions) => {
    let finalOpts = {
    ...
    const idpList = await listApplications(finalOpts);
    return {
      data: idpList.items,
      total: idpList.total,
    };
  ...
  const handleEditApp = (instanceId: string) => {
    history.push(`/app-management/app/edit/${instanceId}`);
  };

  const deleteModalConfig = (records: App.Application[]) => {
    let title =
      records.length === 1
    ...
    return {
      ...
      centered: true,
      onOk: () => {
        doDeleteApp(records[0].metadata.instanceId);
      },
  ...
  const columns: ProColumns<App.Application>[] = [
    {
      ...
      render: (_, record: App.Application) => (
        <a key="instanceId" onClick={() => handleEditApp(record.metadata.instanceId)}>
    ...
    {
      title: <FormattedMessage {...INTL.LOGO} />,
      dataIndex: 'logo',
      hideInSearch: true,
      width: 90,
    },
    {
      title: <FormattedMessage {...INTL.HOMEPAGE_URL} />,
